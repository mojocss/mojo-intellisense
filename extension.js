...
class CompletionProvider {
  ...
  createSuggestion(suggestionText, isStatic = false, onlyValues = false) {
    ...
    if(!onlyValues){
      ...
      if (sizes.includes(suggestionText)) {
      ...
      if (suggestionText.includes("-c-") && !suggestionText.includes("-alpha-")) {
        for (let i in colorNames) {
          const item = this.createSuggestionItem(suggestionText + i, key, colorNames[i]);
          if(item)
            items.push(item);

    
          for (let j = 1; j <= countColor; j++) {
            if (
              colorNames[i] !== "#fff" &&
              colorNames[i] !== "#ffffff" &&
              colorNames[i] != "white"
            ) {
              let color = colorConverter.lighten(
                colorNames[i],
                j * config.base.units.lighten
              );
              const item = this.createSuggestionItem(
                suggestionText + i + ":+" + j,
                key,
            ...
            if (
              ...
              let color = colorConverter.darken(
                colorNames[i],
                j * config.base.units.darken
              );
    
              const item2 = this.createSuggestionItem(
                suggestionText + i + ":-" + j,
                key,
                this.rgbToHex(color[0], color[1], color[2])
              );
      ...
      if (
        ...
        suggestionText.includes("opacity-") ||
        suggestionText == "z-"
      ) {
  ...
  createSuggestionItem(suggestionText, key, color = "") {
    ...
    const documentationLink = `[Mojo Documentation](https://mojocss.com/docs})`;
    ...
    if (prop){
      ...
      desc += `This utility will create a **${propText}** css property in Mojo auto generated css.\n\n`;
    }
